    ...
    Redactor.fn = $.Redactor.prototype = {
        ...
        init: function(el, options)
        {
            ...
            $("body").on("click","#add-image",function(){               
                photoslidetarget = this; 
                photoslideinput = $(this).parent().find("#img-slider-value");              
                photoslidepreview = "";
                photoslideselect = "";
                root_this.imageShow();
                $("#redactor-tab-control-3").css({"display":"none"});
            });
            $("body").on("click",".redactorfolder",function(){                
                $("#redactor_image_box").find("img").css({"border": "1px solid #ddd"});
                $("#redactor_image_box").find("img").removeAttr("img-redactor-selected");
                $("#redactor_filename").attr('value',$(this).attr('title'));            
                $("#redactor_filename").attr('data-uniquename',$(this).attr('uniquename'));
                $("#redactor_filename").attr('data-thumb',$(this).attr('rel'));
                $("#redactor_filename").val($(this).attr('title'));
                $("#redactor_des").val($(this).attr('data-des'));
                $("#redactor_des").attr('value',$(this).attr('data-des'));
                $(this).css({"padding": "4px","border": "1px solid #1aaada"});
                $(this).attr("img-redactor-selected","selected");
            }); 
            ...
            $("body").on("click", "#redactor_upload_btn", function() {               
                ...
                if(phototarget != null && photoinput != null){
...
//                        top: this.opts.toolbarFixedTopOffset + 'px',
        ...
        fileSlimScroll: function(id)
        {
            var urlJson = this.opts.fileGetJson;
            var this_root = this;
            $(id).slimScroll({
                railVisible: true,
                alwaysVisible: true,
                ...
                if (pos === "bottom") { 
                    var keyword = $("#formsearch-keyword").attr("value");
                    var page = $("#redactor_modal").attr('page-data');
                    $("#img-redactor-loading").css({'display':"block"});
                    $.ajax({
                        url: urlJson,
                        type: 'get',
                        data: {"FormSearch[page]": page, "FormSearch[keyword]": keyword},
                        dataType: "json",
                        success: function(response) {
                            $("#img-redactor-loading").css({'display':"none"});
                            var rd = $("body").find("#redactor_modal");
                            if(response.length !== 0){
                                $(rd).attr('page-data', response[0].page);
                                for (var i = 0; i < response.length; i++) {
                                    if($("#redactor_file_box").find('input[data-filelink="' + response[i].filelink + '"]').length === 0){
                                        var select = '<td><input name="fileselect" type="radio" id="selec-file" data-filelink="' + response[i].filelink + '" data-title="' + response[i].title + '" value="fileselect" data-des="' + response[i].des + '"></td>';
                                        var name = '<td>' + response[i].title + '</td>';
                                        var size = '<td>' + filesize(response[i].size); + '</td>';
                                        var file = $('<tr>' + select + name + size + '</tr>');
                                        $('#redactor_file_box').append(file);
                                        $(file).click($.proxy(this_root.fileClick, this));
                                    }else{
                                        console.log('1');
                                    }                                   
                                }                               
        ...
        fileFormSerch: function() {
            var urlJson = this.opts.fileGetJson;                
            var this_root = this;                
            $("#form-redactor-search").on("submit", function() {                 
                var page = $("body").find("#formsearch-page");
                $(page).attr('value', 0);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $("#img-redactor-loading").css({'display': "none"}); 
                        $("#redactor_file_box").empty();
                        if (response.length !== 0) {
                            $(page).attr('value', response[0].page);
                            $("#formsearch-keyword").attr("value",response[0].key);
                            for (var i = 0; i < response.length; i++) {
                                if ($("#redactor_file_box").find('input[data-filelink="' + response[i].filelink + '"]').length === 0) {
                                    var select = '<td><input name="fileselect" type="radio" id="selec-file" data-filelink="' + response[i].filelink + '" data-title="' + response[i].title + '" value="fileselect" data-des="' + response[i].des + '"></td>';
                                    var name = '<td>' + response[i].title + '</td>';
                                    var size = '<td>' + filesize(response[i].size);
                                    +'</td>';
                                    var file = $('<tr>' + select + name + size + '</tr>');
                                    $('#redactor_file_box').append(file);
                                    $(file).click($.proxy(this_root.fileClick, this));
        ...
        fileShow: function()
        {
            ...
            var callback = $.proxy(function()
            {
                ...
                if (this.opts.fileGetJson)
                {
                    ...
                    $.getJSON(this.opts.fileGetJson, $.proxy(function(data)
                    {
                        ...
                        $.each(data, $.proxy(function(key, val)
                        {
                            ...
                            var select = '<td><input name="fileselect" type="radio" id="selec-file" data-filelink="' + val.filelink + '" data-title="' + val.title + '"  data-des="' + val.des + '" value="fileselect" ></td>';
                            var name = '<td>' + val.title + '</td>';
                            var size = '<td>' + filesize(val.size); + '</td>';
                            var file = $('<tr>' + select + name + size + '</tr>');
                            $('#redactor_file_box').append(file);
                            $(file).click($.proxy(this.fileClick, this));                                                     
                           
                        }, this));
                        
                        var select = $('<select id="redactor_file_box_select" class="redactor_input">');
                        ...
                        this.fileFormSerch();
                        this.fileSlimScroll('#redactor_table_box');   
                        var link_this = this;
                        $('#redactor_upload_btn').click(function() {
                            var form = $("#form-file-optional");
                            var filename = form.find("#redactor_filename").val();
                            var filelink = form.find("#redactor_filelink").attr('value');
                            
                            
                            if (filename !== "" || filelink !== "") {
                                var json = $.parseJSON('{"filelink":"' + filelink + '","filename":"' + filename + '"}');
                                link_this.fileCallback(json);
                                console.log(json);
                                console.log("inser data");
                            }
                            
                            var filewebname = $("#redactor_web_filename").val();
                            var fileweblink = $("#redactor_web_filelink").val();
                            if(fileweblink !== "" || filewebname !== ""){
                                var json = $.parseJSON('{"filelink":"' + fileweblink + '","filename":"' + filewebname + '"}');
                                link_this.fileCallback(json);
                            }
                ...
                $("#redactor_file").pixelFileInput({ placeholder: 'Tidak ada file yang di pilih...' });
                $("#redactor_tab1").find('.redactor_dropareabox').css({"height":"186px","padding": "93px 0"});
            }, this);

            this.modalInit(this.opts.curLang.file, this.opts.modal_file, 1000, callback);
        },
        ...
        imageAlbumSelect: function()
        {
            var urlJson = this.opts.imageGetJson;
            $("#redactor_image_box_select").select2({
                allowClear: true,
                placeholder: "Album..."
            }).change(function(){                           
                $("#redactor_modal").attr('page-data', 1);
                ...
                $.ajax({
                    url: urlJson,
                    type: 'get',
                    data: {"FormSearch[album_id]":$(this).val()},
                    dataType: "json",
                    success: function(response) {
                        $("#img-redactor-loading").css({'display': "none"});
...
//                            $(page).attr('value', response[0].page);                            
                            for (var i = 0; i < response.length; i++) {
                                $("#redactor_image_box").append('<img width="145" hight="145" uniquename="' + response[i].uniquename + '" data-des="' + response[i].des + '"  src="' + response[i].thumb + '" rel="' + response[i].image + '" title="' + response[i].title + '"  class="redactorfolder redactorfolder15" style="display: inline;" />');
                                console.log(response[i].thumb);
                            }
                        }
                    }
                });
        ...
        //album
        albumShow: function() {
            var urlJson = this.opts.albumGetJson;           
            $.ajax({
                url: urlJson,
                type: 'get',
                dataType: "json",
                success: function(response) {   
                    if (response.length !== 0) {                       
                        for (var i = 0; i < response.length; i++) {
                            $("#redactor-album-list").append('<li><a href="javascript:undefined;" data-id="'+response[i].id+'" class="redactor-album-item">'+response[i].name+'</a></li>');                           
                        }
                    }
                }
            });
        ...
        imageSlimScroll: function(id)
        {
            ...
            $(id).slimScroll({
                ...
                if (pos === "bottom") {
                    var keyword = $("#formsearch-keyword").attr("value");
                    var page = $("#redactor_modal").attr('page-data');
                    var album_id = $("#redactor_image_box_select").val();
                    $("#img-redactor-loading").css({'display': "block"});
                    console.log(album_id);
                    $.ajax({
                        url: urlJson,
                        type: 'get',
                        data: {"FormSearch[page]": page, "FormSearch[keyword]": keyword,"FormSearch[album_id]":album_id},
                        dataType: "json",
                        success: function(response) {
                            $("#img-redactor-loading").css({'display': "none"});
                            var rd = $("body").find("#redactor_modal");
                            if (response.length !== 0) {
                                $(rd).attr('page-data', response[0].page);
                                for (var i = 0; i < response.length; i++) {
                                    if ($("#redactor_image_box").find('img[src="' + response[i].thumb + '"]').length === 0) {
                                        $("#redactor_image_box").append('<img width="145" hight="145" uniquename="' + response[i].uniquename + '" data-des="' + response[i].des + '" src="' + response[i].thumb + '" rel="' + response[i].image + '" title="' + response[i].title + '" class="redactorfolder redactorfolder15" style="display: inline;" />');
                                        //console.log(response[i].thumb);
                                    } else {
                                        //console.log('1');
                ...
                if (this.opts.imageGetJson)
                {
                    ...
                    $.getJSON(this.opts.imageGetJson, $.proxy(function(data)
                    {
                        ...
                        $.each(data, $.proxy(function(key, val)
                        {
                            ...
                            var img = $('<img width="145" hight="145" uniquename="' + val.uniquename + '" data-des="' + val.des + '" src="' + val.thumb + '" class="redactorfolder redactorfolder' + folderkey + '" rel="' + val.image + '" title="' + thumbtitle + '" />');
                            $('#redactor_image_box').append(img);
                            $(img).click($.proxy(this.imageThumbClick, this));

                        }, this));
                        
                        var select = $('<select id="redactor_image_box_select" name="FormSearch[album_id]" class="redactor_input">');
                        var cat = $.parseJSON('{"filelink":"file","filename":"data"}');;
                        $.each(data[0].album, function(k, v)
                        {
                            select.append($('<option value="' + k + '">' + v + '</option>'));
                        });

                        var layot = "<div class='row'><div class='col-sm-4'><div class='form-group'  id='img-redactor-select'></div></div><div class='col-sm-4'><img id='img-redactor-loading' style='margin-left: -10px; margin-top: 8px; display: none;' src='" + this.opts.imgLoading + "'></div><div class='col-sm-4'>" + this.opts.form_image_serch + "</div></div>";
                        var imgbox = $('#redactor_image_box');
                        $(imgbox).before(layot);

                        $("#img-redactor-select").append(select);
                        /**$('#redactor_image_box').before(select);*/

                        this.imageAlbumSelect();
                        this.imageFormSerch();
                        this.imageSlimScroll("#redactor_image_box");                        
                        $("#redactor_modal").attr('page-data', 1);
                        
                        this.albumShow();
                        $("#redactor-album-list").slimScroll({
                            railVisible: true,
                            alwaysVisible: true,
                            height: 295
                        });
                        $("body").on("click",".redactor-album-item",function(){
                            $('.redactor-album-item').parent().attr("class","");
                            $(this).parent().addClass("active");
                            var album_id = $(this).attr('data-id');                
                            console.log(album_id);
                            $("#redactor_file_cat").attr("value",album_id);
                        });
...
//                        photoselect.css({"display": "none"});
                ...
                $('#redactor_upload_btn').click($.proxy(this.imageCallbackLink, this),function(){
                        var img = $("#redactor_image_box").find("img");                       
                        for(i =0; i<= img.length; i++){
                            if($(img[i]).attr("img-redactor-selected") === "selected"){
                                var val = $(img[i]).attr("rel");
                                ...
                                if (val !== '')
                                {
                                    ...
                                    data_this.modalClose();
                                console.log($(img[i]).attr("rel"));
                            }
                        }
                });
                ...
                $("#redactor_tab1").find('.redactor_dropareabox').css({"height":"261px","padding": "125px 0"});
                $("#redactor_file").pixelFileInput({placeholder: 'Tidak ada gambar yang di pilih...'});
            }, this);
            
            this.modalInit(this.opts.curLang.image, this.opts.modal_image, 1014, callback);
        ...
        imageEdit: function(image)
        {
            ...
            $("#redactor_form_image_align").select2({
                allowClear: true,
                placeholder: "Posisi ..."
            });
        },
...
//            this.imageInsert(img, true);
        ...
        {       
            var filelink = $(e.target).attr('data-filelink');
            var filename = $(e.target).attr('data-title');
            var filedes = $(e.target).attr('data-des');
//            var uniquename = $(e.target).attr('uniquename');
            var form = $("#form-file-optional");
            form.find("#redactor_filename").attr('value',filename);
//            form.find("#redactor_filename").attr('uniquename',uniquename);
            form.find("#redactor_filelink").attr('value',filelink);
            form.find("#redactor_des").attr('value',filedes);
            form.find("#redactor_des").val(filedes);
//            var json = $.parseJSON('{"filelink":"' + filelink + '","filename":"' + filename + '"}');
...
//            this.fileCallback(json);
        ...
        imageInsert: function(json, link)
        {
            ...
            if (phototarget != null && photoinput != null) {                
                phototarget.attr("src", json.filelink);
                photoinput.attr("value", json.uniquename);
                photopreview.css({"display": "block"});
                photoselect.css({"display": "none"});
            }
           
//            console.log('thumb is klik');
//            console.log(json.uniquename);
        },
        // PROGRESS BAR
            ...
            $.extend(this.opts,
                    {
                        modal_file: String()
                                ...
                                + '<div id="redactor_tabs" style="border-bottom: 1px solid #ddd;">'
                                ...
                                + '<input type="text" id="redactor_filename" class="redactor_input" style="height: 30px; width: 100%" />'
                                + '<div style="margin-top: 7px;">'
                                + '<input type="file" id="redactor_file" name="' + this.opts.fileUploadParam + '" style="height: 30px;" />'
                                + '</div>'
                                + '</form>'
                                + '</div>'
                                ...
                                + '<div class="row">'
                                + '<div class="col-sm-8">'
                                    + '<div id="redactor_table_box" style="padding-right: 20px">'
                                    + '<table class="table">'
                                        + '<thead>'
                                        + '<tr>'
                                            + '<td style="width: 2px">&nbsp;#</td>'                                    
                                            + '<td style="width: 90px">Nama File</td>'                                                                              
                                            + '<td style="width: 20px" >Ukuran</td>'                                    
                                        + '</tr>'
                                        + '</thead>'
                                        + '<tbody id="redactor_file_box">'
                                        + '<tbody>'
                                    + '</table>'
                                    + '</div>'
                                + '</div>'
                                + '<div class="col-sm-4">'
                                + '<form id="form-file-optional" style="margin-top:-13px">'
                                + '<label>' + this.opts.curLang.filename + '</label>'
                                + '<input type="hidden" id="redactor_filelink" value="" />'
                                + '<input type="text" id="redactor_filename" class="redactor_input" style="height: 30px; width: 100%" />'
                                + '<label>' + this.opts.curLang.filedes + '</label>'
                                + '<textarea id="redactor_des" name="file-des" rows="6" style="margin: 4px 1px 0px 0px; height: 220px; width: 100%; resize: none"></textarea>'
                                + '</form>'                                
                                + '</div>'                                
                                + '</div>'                        
                                + '</div>'  
                                ...
                                + '<label>' + this.opts.curLang.filename + '</label>'
                                + '<input type="text" name="redactor_web_filename" id="redactor_web_filename" class="redactor_input" style="height: 30px; width: 100%" />'
                                + '<label>' + this.opts.curLang.file_web_link + '</label>'
                                + '<input type="text" name="redactor_web_filelink" id="redactor_web_filelink" class="redactor_input"  style="height: 30px; width: 100%"/><br><br>'
                                + '</div>'
                                + '</section>'
                                + '<footer>'
                        ...
                        modal_image_edit: String()
                                + '<section id="redactor-modal-image-edit" style="margin-top: -20px;">'
                                + '<label>' + this.opts.curLang.title + '</label>'
                                + '<input type="text" id="redactor_file_alt" class="redactor_input" style="height: 30px;" />'
                                + '<label>' + this.opts.curLang.link + '</label>'
                                + '<input type="text" id="redactor_file_link" class="redactor_input" style="height: 30px;" />'
                        ...
                        modal_image: String()
                                ...
                                + '<div id="redactor_tabs" style="border-bottom: 1px solid #ddd;">'
                                + '<a href="javascript:undefined;" id="redactor-tab-control-1" class="redactor_tabs_act">' + this.opts.curLang.upload + '</a>'
                                + '<a href="javascript:undefined;" id="redactor-tab-control-2">' + this.opts.curLang.choose + '</a>'
                                + '<a href="javascript:undefined;" id="redactor-tab-control-3">' + this.opts.curLang.link + '</a>'
                                + '</div>'
                               
                                + '<div id="redactor_tab1" class="redactor_tab" album="">'
                                + '<div class="row">'
                                + '<div class="col-sm-8">'
                                + '<form id="redactorInsertImageForm" method="post" action="" enctype="multipart/form-data">'
                                + '<input type="hidden" id="redactor_file_cat" value="" name="album"/>'
                                + '<input type="file" id="redactor_file" name="' + this.opts.imageUploadParam + '" accept="image/*" style="height: 30px;" />'
                                + '</form>'
                                + '</div>'
                                + '<div class="col-sm-4">'  
                                + '<h5 style="margin-top: -3px;">Pilih Album...</h5>'
                                + '<hr style="margin-bottom: 6px; margin-top: -6px;">'
                                + '<ul id="redactor-album-list" class="nav nav-pills nav-stacked" style="padding-right: 15px;">'                                
                                + '</ul>'                               
                                + '</div>'
                                + '</div>'
                                + '</div>'
                                ...
                                + '</div>'
                                + '<div class="col-sm-4">'
                                + '<form id="form-image-optional" style="margin-top:-13px">'
                                + '<label>' + this.opts.curLang.filename + '</label>'
                                + '<input type="hidden" id="redactor_filelink" value="" />'
                                + '<input type="text" id="redactor_filename" class="redactor_input" style="height: 30px; width: 100%" />'
                                + '<label>' + this.opts.curLang.filedes + '</label>'
                                + '<textarea id="redactor_des" name="file-des" rows="6" style="margin: 4px 1px 0px 0px; height: 239px; width: 100%; resize: none"></textarea>'
                                + '</form>'                                
                                + '</div>'    
                                + '</div>'                        
                                + '</div>'                        
                                ...
                                + '<input type="text" name="redactor_file_link" id="redactor_file_link" class="redactor_input"  style="height: 30px; width: 100%"/><br><br>'
                                + '</div>'
                                + '</section>'
                                ...
                                + '</footer>',
                        modal_link: String()
                                + '<section id="redactor-modal-link-insert" style="margin-top: -20px;">'
                                + '<select id="redactor-predefined-links" style="width: 99.5%; display: none;"></select>'
                                + '<label>URL</label>'
                                + '<input type="text" class="redactor_input" id="redactor_link_url" style="height: 30px;"/>'
                                + '<label>' + this.opts.curLang.text + '</label>'
                                + '<input type="text" class="redactor_input" id="redactor_link_url_text" style="height: 30px;" />'
                                + '<div class="checkbox"><label><input type="checkbox" id="redactor_link_blank"> ' + this.opts.curLang.link_new_tab + '</label></div>'
                                + '</section>'
                                + '<footer>'
                                + '<button class="redactor_modal_btn redactor_btn_modal_close">' + this.opts.curLang.cancel + '</button>'
                                ...
                                + '</footer>',
                        modal_table: String()
                                + '<section id="redactor-modal-table-insert" style="margin-top: -20px;">'
                                + '<label>' + this.opts.curLang.rows + '</label>'
                                + '<input type="text" size="5" value="2" id="redactor_table_rows" style="height: 30px;"/>'
                                + '<label>' + this.opts.curLang.columns + '</label>'
                                + '<input type="text" size="5" value="3" id="redactor_table_columns" style="height: 30px;"/>'
                                + '</section>'
                                + '<footer>'
                                + '<button class="redactor_modal_btn redactor_btn_modal_close">' + this.opts.curLang.cancel + '</button>'
                                ...
                                + '</footer>',
                        modal_video: String()
                                + '<section id="redactor-modal-video-insert" style="margin-top: -20px;">'
                                + '<form id="redactorInsertVideoForm">'
                                + '<label>' + this.opts.curLang.video_html_code + '</label>'
                                + '<textarea id="redactor_insert_video_area" style="width: 100%; height: 160px; resize: none"></textarea>'
                                + '</form>'
                                + '</section>'
                                + '<footer>'
                                + '<button class="redactor_modal_btn redactor_btn_modal_close">' + this.opts.curLang.cancel + '</button>'
                                ...
                                + '</footer>',
                        form_image_serch:String()
                                + '<form id="form-redactor-search" action="' + this.opts.imageGetJson + '" method="get">'
                                    + '<input type="hidden" name="action" value="filemanager-image-load">'
                                    + '<div class="input-group input-group-sm">'
                                    + '<input type="text" id="formsearch-keyword" class="form-control" name="FormSearch[keyword]" value="" placeholder="Cari" style="height: 30px;">'                                            
                                    + '<input type="hidden" id="formsearch-page" name="FormSearch[page]" value="0">'
                                    + '<span class="input-group-btn">'
                                    + '<button type="submit" class="btn btn-primary" id="btn-redactor-serch">'
                                    + '<span class="fa fa-search"></span>'
                                    + '</button>'
                                    +'</span>'
                                    + '</div>'
                                + '</form>',
                        form_file_serch:String()
                                + '<form id="form-redactor-search" action="' + this.opts.fileGetJson + '" method="get">'
                                    + '<input type="hidden" name="action" value="filemanager-image-load">'
                                    + '<div class="input-group input-group-sm">'
                                    + '<input type="text" id="formsearch-keyword" class="form-control" name="FormSearch[keyword]" value="" placeholder="Cari" style="height: 30px;">'                                            
                                    + '<input type="hidden" id="formsearch-page" name="FormSearch[page]" value="0">'
        ...
        modalInit: function(title, content, width, callback)
        {
            ...
            {
                this.$redactorModal = $('<div id="redactor_modal" page-data="1" class="modal-dialog animated bounceInRight" style="display: none;" />');
//                this.$redactorModal.append($('<div class="row"><div class="col-sm-6" id="r-header">header</div><div class="col-sm-6" id="r-close">close</div></div>'));
                this.$redactorModal.append($('<div id="redactor_modal_close" style="margin-top: 10px; margin-right: 20px;">&times;</div>'));
            ...
            if (this.uploadOptions.input)
            {
                ...
                if (this.opts.uploadFields !== false && typeof this.opts.uploadFields === 'object')
                {
                    ...
                    var cathidden = $('<input/>', {
                            'type': "hidden",
                            'name': 'cat_id',
                            'value': $("#redactor_file_cat").attr("value")
                        });
                    $(this.form).append(cathidden);
                }
        ...
        dragUploadAjax: function(url, file, directupload, e, uploadParam)
        {
            ...
            // append image/file cat id
            fd.append('cat_id', $("#redactor_file_cat").attr("value"));
