...
class MemberModel extends Member
{
    ...
    public function rules()
    {
        return [
            ...
            isset(\Yii::$app->user->identity->role) == "paskibramember" ? [['name',
            'nickname',
            'address',
            'birth',
            'nationality',
            ...
            'shoe_size',
            'hat_size'
                ], 'required', 'message' => 'Tidak boleh kosong.'] : [['nra'], 'safe'],
            isset(\Yii::$app->user->identity->role) == "ppimember" ? [['name',
            'nickname',
            'address',
            'birth',
            'nationality',
            ...
            isset(\Yii::$app->user->identity->role) == "capasmember" ? [['name',
            'nickname',
            'address',
            'birth',
            'nationality',
    ...
    {
        return $this->hasMany(\app\modules\dao\ar\Taxonomy::className(), ['id' => 'taxonomy_id'])
    ...
    public function saveTaxRelation($data = [], $member_id)
    {
        ...
        foreach ($data as $id) {
            $new = new \app\modules\dao\ar\Taxmemberrelations();
    ...
    {
        foreach ($data as $id) {
            \app\modules\dao\ar\Taxmemberrelations::deleteAll([
                'taxonomy_id' => $id,
                'member_id' => $member_id
            ]);
        }
    ...
    {
        $query = \app\modules\member\models\LifeSkillModel::findOne($id);
        return $query->name;
    }
    ...
    {
        $query = \app\modules\member\models\LanguageSkillModel::findOne($id);
        return $query->name;
    }
    ...
    {
        $query = \app\modules\member\models\BrevetAwardModel::findOne($id);
        return $query->name;
    }
