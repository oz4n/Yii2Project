...
/**
 * CategorySerch represents the model behind the search form about `app\modules\dao\ar\Taxonomy`.
 */
class CategorySerch extends CategoryModel
{
    ...
    {
        $query = self::find();
        $query->onCondition(['term_id' => Word::WORD_CATEGORY]);
...
//        ]);
        ...
        if (isset($params['CategorySerch']['keyword'])) {
            $this->keyword = $params['CategorySerch']['keyword'];
...
//            'lft' => SORT_ASC
//        ]);
        $models->where(['term_id' => Word::WORD_CATEGORY]);
    ...
    {
        $models = self::find();
        $models->where(['parent_id' => null, 'term_id' => Word::WORD_CATEGORY]);
