...
/**
 * TagSerch represents the model behind the search form about `app\modules\dao\ar\TagModel`.
 */
class TagSerch extends TagModel
{
    ...
    {
        $query = self::find();
        $query->onCondition(['term_id' => Word::WORD_TAG]);
...
//        ]);
        ...
        if (isset($params['TagSerch']['keyword'])) {
            $this->keyword = $params['TagSerch']['keyword'];
...
//            'lft' => SORT_ASC
//        ]);
        $models->where(['term_id' => Word::WORD_TAG]);
    ...
    {
        $models = self::find();
        $models->where(['parent_id' => null, 'term_id' => Word::WORD_TAG]);
