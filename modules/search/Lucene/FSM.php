...
use app\modules\search\Lucene\FSMAction;
use app\modules\search\Lucene\Exception;
...
abstract class FSM
{
    ...
    public function setState($state)
    {
        if (!isset($this->_states[$state])) {           
            throw new Exception('State \'' . $state . '\' is not on of the possible FSM states.');
        }
    ...
    public function addRule($sourceState, $input, $targetState, $inputAction = null)
    {
        if (!isset($this->_states[$sourceState])) {
            ...
            throw new Exception('Undefined source state (' . $sourceState . ').');
        }
        if (!isset($this->_states[$targetState])) {
            
            throw new Exception('Undefined target state (' . $targetState . ').');
        }
        if (!isset($this->_inputAphabet[$input])) {
            
            throw new Exception('Undefined input symbol (' . $input . ').');
        }
        ...
        if (isset($this->_rules[$sourceState][$input])) {
            ...
            throw new Exception('Rule for {state,input} pair (' . $sourceState . ', '. $input . ') is already defined.');
        }
    ...
    public function addInputAction($state, $inputSymbol, FSMAction $action)
    {
        ...
        if (!isset($this->_inputAphabet[$inputSymbol])) {
            ...
            throw new Exception('Undefined input symbol (' . $inputSymbol. ').');
        }
    ...
    public function addTransitionAction($sourceState, $targetState, FSMAction $action)
    {
        ...
        if (!isset($this->_states[$targetState])) {
            ...
            throw new Exception('Undefined source state (' . $targetState. ').');
        }
    ...
    public function process($input)
    {
        if (!isset($this->_rules[$this->_currentState])) {
            ...
            throw new Exception('There is no any rule for current state (' . $this->_currentState . ').');
        }
        if (!isset($this->_rules[$this->_currentState][$input])) {
            
            throw new Exception('There is no any rule for {current state, input} pair (' . $this->_currentState . ', ' . $input . ').');
        }
    ...
    public function reset()
    {
        if (count($this->_states) == 0) {
            ...
            throw new Exception('There is no any state defined for FSM.');
        }
